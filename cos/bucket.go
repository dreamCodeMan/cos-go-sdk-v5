...
// 获得云存储上文件信息
func (b *Bucket) GetObjectMeta(ctx context.Context, object string) (http.Header, error) {
	resq, err := b.conn.Do(ctx, "HEAD", b.Name, object, nil, nil, nil)
	if err != nil {
		return nil, err
	}
	...
	return resq.Header, nil
}

// 获得云存储上文件信息
func (b *Bucket) GetObjectList(prefix, delimiter, marker string, maxKeys int) (*ListObjectsResult, error) {
	...
	resq, err := b.conn.Do(GetTimeoutCtx(time.Second*30), "GET", b.Name, "", params, nil, nil)
	if err != nil {
		return listObjects, err
	}
	defer resq.Body.Close()

	//result, err := ioutil.ReadAll(resq.Body)
	//fmt.Println(string(result), err)

	err = XMLDecode(resq.Body, listObjects)
	if err != nil {
		return listObjects, err
	}
